...
struct TextDecoder: EditorContentDecoding {
    func decode(mode: EditorContentMode, maxSize: CGSize, value: JSON) -> NSAttributedString {
		...
		if let underline = value["NSUnderline"] as? Int {
			string.addAttribute(NSAttributedString.Key.underlineStyle, value: underline, range: string.fullRange)
		}
		if let strikethrough = value["NSStrikethrough"] as? Int {
			string.addAttribute(NSAttributedString.Key.strikethroughStyle, value: strikethrough, range: string.fullRange)
		}
        return string
    }
}

    ...
    func encode(name: EditorContent.Name, string: NSAttributedString) -> JSON {
        ...
        string.enumerateAttributes(in: string.fullRange, options: .longestEffectiveRangeNotRequired) { (attributes, range, _) in
            ...
            for attr in attributes {
				var encoding: InlineEncoding? = nil
				if let inline = attr.value as? InlineEncoding {
					encoding = inline
				} else if let permittedEncoding = PermittedStylesEncoding(key: attr.key.rawValue, value: attr.value) {
					encoding = permittedEncoding
				}
